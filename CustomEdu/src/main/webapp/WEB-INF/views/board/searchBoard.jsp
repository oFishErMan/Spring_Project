					...
					<div class="panel">
						...
						<div style="padding-left: 20px; padding-right: 20px;">
							<input type="text" style="height: 33px;"
								placeholder="검색어를 입력하세요." id="searchBoard" >
							<input type="button" id="board-search-btn"
								class="btn btn-scondary" value="검색"  onclick="searchBoard()">
							<input type="button" style="float: right; margin-left: 5px;"
								class="btn btn-scondary" onclick="delBoard()" value="삭제">
							<button type="button" style="float: right;"
								class="btn btn-scondary" onclick="location.href='/board/regBoard'">글쓰기</button>
						</div>
						<div class="panel-body">
								...
								<tbody>
									<c:forEach items="${list}" var="list">
										<tr>
											...
											<td id="board_seq" ><c:out value="${list.board_seq}" /></td>
							...
							<div style="text-align: center; display: block; border: 10px;">
								<c:if test="${paging.startPage != 1 }">
									<a
										href="/board/boardList?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
								</c:if>
								<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
									...
									<c:choose>
										...
										<c:when test="${p != paging.nowPage }">
											<a
												href="/board/boardList?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
										</c:when>
									</c:choose>
								</c:forEach>
								<c:if test="${paging.endPage != paging.lastPage}">
									<a
										href="/board/boardList?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
								</c:if>
							</div>
						</div>
					</div>
